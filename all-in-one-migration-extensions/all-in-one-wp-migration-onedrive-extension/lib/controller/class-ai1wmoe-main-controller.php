...
class Ai1wmoe_Main_Controller {
	...
	public function create_activation_request( $purchase_id ) {
		...
		if ( defined( 'AI1WMOE_PURCHASE_ACTIVATION_URL' ) ) {
			wp_remote_post(
				AI1WMOE_PURCHASE_ACTIVATION_URL,
				array(
					'timeout' => 15,
					'body'    => array(
						'url'           => get_site_url(),
	...
	public function enqueue_export_scripts_and_styles( $hook ) {
		...
		if ( ! defined( 'AI1WMUE_PLUGIN_NAME' ) ) {
			wp_enqueue_script(
				...
				Ai1wm_Template::asset_link( 'javascript/file-excluder.min.js', 'AI1WMOE' ),
				array( 'jquery' )
			);
			...
			wp_localize_script(
				...
				'ai1wmue_locale',
				array(
					'button_done'                        => __( 'Done', AI1WMOE_PLUGIN_NAME ),
					'loading_placeholder'                => __( 'Listing files ...', AI1WMOE_PLUGIN_NAME ),
					'selected_no_files'                  => __( 'No files selected', AI1WMOE_PLUGIN_NAME ),
					'selected_multiple'                  => __( '{x} files and {y} folders selected', AI1WMOE_PLUGIN_NAME ),
					'selected_multiple_folders'          => __( '{y} folders selected', AI1WMOE_PLUGIN_NAME ),
					'selected_multiple_files'            => __( '{x} files selected', AI1WMOE_PLUGIN_NAME ),
					'selected_one_file'                  => __( '{x} file selected', AI1WMOE_PLUGIN_NAME ),
					'selected_one_file_multiple_folders' => __( '{x} file and {y} folders selected', AI1WMOE_PLUGIN_NAME ),
					'selected_one_file_one_folder'       => __( '{x} file and {y} folder selected', AI1WMOE_PLUGIN_NAME ),
					'selected_one_folder'                => __( '{y} folder selected', AI1WMOE_PLUGIN_NAME ),
					'selected_multiple_files_one_folder' => __( '{x} files and {y} folder selected', AI1WMOE_PLUGIN_NAME ),
					'column_name'                        => __( 'Name', AI1WMOE_PLUGIN_NAME ),
					'column_date'                        => __( 'Date', AI1WMOE_PLUGIN_NAME ),
					'legend_select'                      => __( 'Click checkbox to toggle selection', AI1WMOE_PLUGIN_NAME ),
					'legend_expand'                      => __( 'Click folder name to expand', AI1WMOE_PLUGIN_NAME ),
					'error_message'                      => __( 'Something went wrong, please refresh and try again', AI1WMOE_PLUGIN_NAME ),
					'button_clear'                       => __( 'Clear selection', AI1WMOE_PLUGIN_NAME ),
					'empty_list_message'                 => __( 'Folder empty. Click on folder icon to close it.', AI1WMOE_PLUGIN_NAME ),
				)
			);
			...
			wp_localize_script(
				...
				array(
					'ajax' => array(
						'url'        => wp_make_link_relative( admin_url( 'admin-ajax.php?action=ai1wmoe_file_list' ) ),
						'list_nonce' => wp_create_nonce( 'ai1wmoe_list' ),
					),
				)
			);
		}
	...
	public function get_missing_dependencies() {
		...
		if ( ! extension_loaded( 'curl' ) ) {
			$messages[] = __( 'Your PHP is missing cURL extension. <a href="https://help.servmask.com/knowledgebase/curl-missing-in-php-installation/" target="_blank">Technical details</a>', AI1WMOE_PLUGIN_NAME );
		}
	...
	public function ai1wm_export_options() {
		if ( ! defined( 'AI1WMUE_PLUGIN_NAME' ) ) {
			...
			if ( ! has_action( 'ai1wm_export_advanced_settings' ) ) {
				add_action( 'ai1wm_export_advanced_settings', 'Ai1wmoe_Export_Controller::exclude_files' );
			}
		}
	}
	...
	public function init() {
		if ( isset( $_GET['ai1wmoe_token'] ) ) {
			update_option( 'ai1wmoe_onedrive_token', trim( $_GET['ai1wmoe_token'] ) );
	...
	public function router() {
		if ( current_user_can( 'export' ) ) {
			...
			add_action( 'wp_ajax_ai1wmoe_file_list', 'Ai1wmoe_Export_Controller::list_files' );
		}
