...
if ( defined( 'WP_CLI' ) ) {
	class Ai1wmoe_OneDrive_WP_CLI_Command extends WP_CLI_Command {
		public function __construct() {
			if ( ! defined( 'AI1WM_PLUGIN_NAME' ) ) {
				WP_CLI::error_multi_line(
					array(
						__( 'OneDrive Extension requires All-in-One WP Migration plugin to be activated. ', AI1WMOE_PLUGIN_NAME ),
						__( 'You can get a copy of it here: https://wordpress.org/plugins/all-in-one-wp-migration/', AI1WMOE_PLUGIN_NAME ),
					)
				);
				exit;
			}
			...
			if ( is_multisite() && ! defined( 'AI1WMME_PLUGIN_NAME' ) ) {
				WP_CLI::error_multi_line(
					array(
						__( 'WordPress Multisite is supported via our All-in-One WP Migration Multisite Extension.', AI1WMOE_PLUGIN_NAME ),
						__( 'You can get a copy of it here: https://servmask.com/products/multisite-extension', AI1WMOE_PLUGIN_NAME ),
					)
				);
				exit;
			}
			...
			if ( ! is_dir( AI1WM_STORAGE_PATH ) ) {
				if ( ! mkdir( AI1WM_STORAGE_PATH ) ) {
					WP_CLI::error_multi_line(
						array(
							sprintf( __( 'All-in-One WP Migration is not able to create <strong>%s</strong> folder.', AI1WMOE_PLUGIN_NAME ), AI1WM_STORAGE_PATH ),
							__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All-in-One WP Migration plugin to function properly.', AI1WMOE_PLUGIN_NAME ),
						)
					);
					exit;
				}
			...
			if ( ! is_dir( AI1WM_BACKUPS_PATH ) ) {
				if ( ! mkdir( AI1WM_BACKUPS_PATH ) ) {
					WP_CLI::error_multi_line(
						array(
							sprintf( __( 'All-in-One WP Migration is not able to create <strong>%s</strong> folder.', AI1WMOE_PLUGIN_NAME ), AI1WM_BACKUPS_PATH ),
							__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All-in-One WP Migration plugin to function properly.', AI1WMOE_PLUGIN_NAME ),
						)
					);
					exit;
			...
			if ( ! get_option( 'ai1wmoe_onedrive_token', false ) ) {
				WP_CLI::error_multi_line(
					array(
						__( 'In order to use All-in-One WP Migration OneDrive extension you need to configure it first.', AI1WMOE_PLUGIN_NAME ),
						__( 'Please navigate to WP Admin > All-in-One WP Migration > OneDrive Settings and Link your OneDrive account.', AI1WMOE_PLUGIN_NAME ),
					)
				);
				exit;
			}
		...
		public function backup( $args = array(), $assoc_args = array() ) {
			...
			WP_CLI::success( __( 'Backup complete.', AI1WMOE_PLUGIN_NAME ) );
			WP_CLI::log( sprintf( __( 'Backup file: %s', AI1WMOE_PLUGIN_NAME ), ai1wm_archive_name( $params ) ) );
			WP_CLI::log( sprintf( __( 'Backup location: %s', AI1WMOE_PLUGIN_NAME ), $params['archive_url'] ) );
		}
		...
		public function list_backups( $args = array(), $assoc_args = array() ) {
			...
			$backups->setHeaders(
				array(
					'name' => __( 'Backup name', AI1WMOE_PLUGIN_NAME ),
					'date' => __( 'Date created', AI1WMOE_PLUGIN_NAME ),
					'size' => __( 'Size', AI1WMOE_PLUGIN_NAME ),
				)
			);
			...
			foreach ( $items as $item ) {
				if ( pathinfo( $item['name'], PATHINFO_EXTENSION ) === 'wpress' ) {
					$backups->addRow(
						array(
							'name' => $item['name'],
							'date' => sprintf( __( '%s ago', AI1WMOE_PLUGIN_NAME ), human_time_diff( $item['date'] ) ),
		...
		public function restore( $args = array(), $assoc_args = array() ) {
			if ( ! isset( $args[0] ) ) {
				WP_CLI::error_multi_line(
					array(
						__( 'A backup name must be provided in order to proceed with the restore process.', AI1WMOE_PLUGIN_NAME ),
						__( 'Example: wp ai1wm onedrive restore migration-wp-20170913-095743-931.wpress', AI1WMOE_PLUGIN_NAME ),
					)
				);
				exit;
			}
			...
			if ( is_null( $file ) ) {
				WP_CLI::error_multi_line(
					array(
						__( 'The backup file could not be located.', AI1WMOE_PLUGIN_NAME ),
						__( 'To list available backups use: wp ai1wm onedrive list-backups', AI1WMOE_PLUGIN_NAME ),
					)
				);
				exit;
			}
		...
		protected function get_folder_id( $assoc_args ) {
			if ( isset( $assoc_args['folder-path'] ) ) {
				...
				if ( empty( $folder_id ) ) {
					WP_CLI::error( sprintf( __( 'Folder \'%s\' not found', AI1WMOE_PLUGIN_NAME ), $path ) );
					exit;
				}
				return $folder_id;
			}
			return get_option( 'ai1wmoe_onedrive_folder_id', null );
		}
	}
}
