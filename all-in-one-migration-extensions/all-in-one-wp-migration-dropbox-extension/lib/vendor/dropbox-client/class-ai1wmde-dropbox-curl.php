...
class Ai1wmde_Dropbox_Curl {
	...
	public function __construct() {
		if ( ! extension_loaded( 'curl' ) ) {
			throw new Ai1wmde_Error_Exception( __( 'Dropbox Extension requires PHP cURL extension. <a href="https://help.servmask.com/knowledgebase/curl-missing-in-php-installation/" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ) );
		}
		...
		$this->set_option( CURLOPT_SSLVERSION, 1 );
	...
	public function make_request( $parse_as_json = false ) {
		...
		if ( $response === false ) {
			if ( ( $errno = curl_errno( $this->handler ) ) ) {
				throw new Ai1wmde_Connect_Exception( sprintf( __( 'Unable to connect to Dropbox. Error code: %s. <a href="https://help.servmask.com/knowledgebase/dropbox-error-codes/#%s" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ), $errno, $errno ) );
			}
		}
		...
		if ( $http_code >= 400 ) {
			if ( ( $data = json_decode( $response, true ) ) ) {
				if ( isset( $data['error']['.tag'] ) ) {
					if ( isset( $data['error_summary'] ) ) {
						throw new Ai1wmde_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/dropbox-error-codes/#%s" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ), $data['error_summary'], $data['error']['.tag'] ) );
					} else {
						throw new Ai1wmde_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/dropbox-error-codes/#%s" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ), $data['error']['.tag'], $data['error']['.tag'] ) );
					}
				}
			}
		}
		...
		if ( $http_code >= 400 ) {
			if ( isset( $this->messages[ $http_code ] ) ) {
				throw new Ai1wmde_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/dropbox-error-codes/#%s" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ), $this->messages[ $http_code ], $http_code ) );
			} else {
				throw new Ai1wmde_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/dropbox-error-codes/#%s" target="_blank">Technical details</a>', AI1WMDE_PLUGIN_NAME ), $http_code, $http_code ) );
			}
		}
